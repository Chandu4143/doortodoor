...
interface TeamPanelProps {
  ...
  onImportData?: (apartments: Apartment[]) => void;
}

export default function TeamPanel({ apartments, onImportData }: TeamPanelProps) {
...
function ShareDataModal({
  ...
  const shareData = generateShareData(apartments);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Share Campaign Data">
      <div className="space-y-4 text-center">
        ...
        <p className="text-xs text-slate-400">
          {apartments.length} campaigns • {Object.values(apartments).reduce((acc, apt) => 
            acc + Object.values(apt.rooms).flat().length, 0
          )} units
        </p>
      </div>
    </Modal>
...
function ImportDataModal({
  ...
  isOpen: boolean;
  onClose: () => void;
  onImport?: (apartments: Apartment[]) => void;
}) {
  const [code, setCode] = useState('');
  const [error, setError] = useState('');

  const handleImport = () => {
    const data = parseShareData(code);
    if (!data) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Import Data">
      <div className="space-y-4">
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Paste the share code from another device to import campaign data.
        </p>
